 ...
 * et reste vide après un JWT invalide — sans passer par un endpoint HTTP
 * métier (qui n'existe pas encore dans {@code financial-service}).
 */
class JwtAuthenticationFilterTest {
    ...
    void doFilterInternal_withValidToken_populatesSecurityContextWithUserContext() throws Exception {
        ...
        String token = Jwts.builder()
                ...
                .claim(JwtClaimNames.PERMISSIONS, List.of("FINANCIAL_READ"))
        ...
        assertThat(authentication.getAuthorities())
                ...
                .containsExactlyInAnyOrder("ROLE_USER", "FINANCIAL_READ");
        verify(chain).doFilter(request, response);
    }

    @Test
    void doFilterInternal_withInvalidToken_leavesSecurityContextEmpty() throws Exception {
        KeyPairGenerator generator = KeyPairGenerator.getInstance("RSA");
        generator.initialize(2048);
    ...
    @Test
    void doFilterInternal_withoutAuthorizationHeader_leavesSecurityContextEmpty() throws Exception {
        KeyPairGenerator generator = KeyPairGenerator.getInstance("RSA");
        generator.initialize(2048);
        JwtAuthenticationFilter filter = new JwtAuthenticationFilter(new JwtVerifier(generator.generateKeyPair().getPublic()));
