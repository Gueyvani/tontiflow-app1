 ...
 * Bearer) et {@link AccessTokenService} (validation du JWT) — aucune
 * logique de parsing ou de vérification cryptographique n'est réimplémentée
 * ici. Le filtre est {@code OncePerRequestFilter} (exécution unique par
 * requête) et strictement stateless : aucune session n'est créée ni lue.</p>
 */
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    /**
     * @param accessTokenService service de validation des Access Token JWT
     */
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response,
        ...
        if (token.isPresent()) {
            try {
                UserContext userContext = accessTokenService.validate(token.get());
