...
/**
 * Vérifie localement, côté {@code notification-service}, les Access Token
 * JWT (RS256) émis par {@code authentication-service}.
 *
 * <p>Ne charge et ne manipule jamais que la clé publique RSA : aucune clé
 * privée n'est accessible ici. Comportement en parité exacte avec
 * {@code AccessTokenService.validate()} (authentication-service),
 * {@code GatewayJwtVerifier.verify()} (api-gateway) et le {@code JwtVerifier}
 * déjà validé de {@code user-service}/{@code tontine-service}/{@code financial-service}/
 * {@code credit-service} : signature RS256 et expiration sont vérifiées,
 * l'émetteur ({@code iss}) ne l'est volontairement pas, conformément au
 * contrat JWT déjà validé.</p>
 */
public final class JwtVerifier {
    ...
    /**
     * Construit un {@link JwtVerifier} en chargeant la clé publique RSA
     * depuis un PEM (format X509). Toute ressource absente ou tout contenu
     * invalide échoue explicitement — aucune valeur par défaut silencieuse.
     */
    public static JwtVerifier fromPublicKeyResource(Resource publicKeyResource) {
        return new JwtVerifier(loadPublicKey(publicKeyResource));
    }
