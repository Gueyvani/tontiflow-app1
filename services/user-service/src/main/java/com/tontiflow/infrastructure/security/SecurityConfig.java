...
/**
 * Configuration Spring Security de {@code user-service} — socle
 * d'authentification JWT local, servant de patron aux futurs microservices
 * aval (tontine, financial, credit, notification).
 *
 * <p>Remplace l'auto-configuration Spring Boot par défaut (utilisateur en
 * mémoire, mot de passe généré aléatoirement) par une chaîne de filtres
 * strictement stateless authentifiant via Access Token JWT (RS256), en
 * parité exacte avec le contrat déjà validé côté {@code authentication-service}
 * et {@code api-gateway}. Aucune règle RBAC métier n'est introduite ici :
 * {@code user-service} ne possède encore aucun contrôleur métier.</p>
 */
@Configuration
@EnableWebSecurity
