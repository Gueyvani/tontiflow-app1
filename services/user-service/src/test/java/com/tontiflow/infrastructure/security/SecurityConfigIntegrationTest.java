...
/**
 * Test d'intégration de la chaîne de sécurité JWT de {@code user-service}
 * ({@link SecurityConfig}).
 *
 * <p>{@code user-service} n'a aucun contrôleur métier ({@link
 ...
 * protégée, ce qui prouve simultanément que la chaîne de filtres rejette
 * correctement et qu'aucun endpoint Actuator sensible n'est jamais exposé).
...
class SecurityConfigIntegrationTest {
    ...
    @Test
    void sensitiveActuatorEndpoint_remainsUnexposed_evenWithValidToken() {
    ...
    // --- ÉTAPE J : matrice JWT / filtre (endpoint protégé) ---
    ...
    void protectedEndpoint_withValidTokenAndForgedTrustHeaders_stillAuthenticatesFromJwtOnly() {
        ...
        headers.set("X-Username", "usurpateur");
        headers.set("X-Email", "usurpateur@tontiflow.test");
        headers.set("X-Roles", "ROLE_ADMIN");
        headers.set("X-Permissions", "ALL");
